...
def register_ct_images(pre_ct_node, post_ct_node):
    ...
    # Perform rigid registration (6 DOF)
    reg = sitk.ImageRegistrationMethod()
    reg.SetMetricAsMattes()
    reg.SetInterpolator(sitk.sitkLinear)
    reg.SetOptimizerAsGradientDescentLineSearch()
    reg.SetNumberOfIterations(100)
    reg.SetRelaxationFactor(0.5)

    initial_transform = sitk.TranslationTransform(post_ct_image.GetDimension())
    ...
    pre_ct_resampled_node = slicer.mrmlScene.AddNewNodeByClass('vtkMRMLScalarVolumeNode')
...
def subtract_images(post_ct_node, pre_ct_resampled_node):
    ...
    subtracted_array = post_ct_array - pre_ct_resampled_array
...
def save_and_visualize(thresholded_node, output_path="contacts_highlighted.nii"):
...
def main():
    ...
    save_and_visualize(thresholded_node, output_path="contacts_highlighted.nii")
