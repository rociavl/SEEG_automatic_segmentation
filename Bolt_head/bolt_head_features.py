...
CONFIG = {
    ...
    'output_dir': r"C:\Users\rocia\Downloads\TFG\Cohort\Bolt_heads\P8_prueba_completa"  
}
def main():
...
def extract_features_from_validated_regions(validated_regions, brain_mask, spacing, origin):
    """
    Extract features from validated bolt head regions.
    
    Args:
        validated_regions (list): List of dictionaries containing region info
        brain_mask (ndarray): 3D array of the brain mask
        spacing (tuple): Voxel spacing (x,y,z) in mm
        origin (tuple): Volume origin (x,y,z) in mm
        
    Returns:
        list: List of dictionaries containing extracted features for each region
    """
    extracted_features = []
    ...
    for region in validated_regions:
        ...
        features = {
            ...
            'centroid_physical': region.get('physical_centroid', (0,0,0)),
            'volume_voxels': region.get('volume', 0),
            'volume_mm3': region.get('volume', 0) * spacing[0] * spacing[1] * spacing[2],
            'principal_axis': region.get('principal_axis', (0,0,1)),
            'surface_distance': region.get('surface_distance', 0),
            'entry_point': region.get('brain_entry_point', None),
            'entry_distance': region.get('entry_distance', 0)
        }
        
        # Calculate additional spatial features if entry point exists
        if features['entry_point'] is not None:
            # Distance between centroid and entry point
            centroid_to_entry = np.linalg.norm(
                np.array(features['centroid_physical']) - 
                np.array(features['entry_point']))
            features['centroid_to_entry_distance'] = centroid_to_entry
            
            # Angle between principal axis and centroid-to-entry vector
            if np.linalg.norm(features['principal_axis']) > 0:
                centroid_to_entry_vec = (np.array(features['entry_point']) - 
                                        np.array(features['centroid_physical']))
                centroid_to_entry_vec = centroid_to_entry_vec / np.linalg.norm(centroid_to_entry_vec)
                principal_axis_norm = features['principal_axis'] / np.linalg.norm(features['principal_axis'])
                angle_rad = np.arccos(np.clip(np.dot(principal_axis_norm, centroid_to_entry_vec), -1.0, 1.0))
                features['axis_entry_angle_deg'] = np.degrees(angle_rad)
            else:
                features['axis_entry_angle_deg'] = 0
        
        # PCA features if coordinates are available
        if 'coords' in region:
            try:
                pca = PCA(n_components=3)
                pca.fit(region['coords'])
                features['pca_variance_ratio'] = pca.explained_variance_ratio_.tolist()
                features['pca_components'] = pca.components_.tolist()
            except:
                features['pca_variance_ratio'] = [0,0,0]
                features['pca_components'] = [[0,0,0],[0,0,0],[0,0,0]]
...
def create_markups_from_entry_points(validated_regions, output_dir, name="EntryPoints"):
    """
    Create markup fiducials from validated entry points and save to file.
    
    Args:
        validated_regions (list): List of dictionaries containing region info
        output_dir (str): Directory to save the markup file
        name (str): Base name for the output files
        
    Returns:
        vtkMRMLMarkupsFiducialNode: The created markup node
    """
    ...
    # Add each entry point as a fiducial
    for i, region in enumerate(validated_regions):
        if 'brain_entry_point' in region and region['brain_entry_point'] is not None:
            entry_point = region['brain_entry_point']
            markups_node.AddControlPoint(entry_point, f"Bolt_{i+1}")
    
    # Set display properties
    markups_node.GetDisplayNode().SetTextScale(4)
    markups_node.GetDisplayNode().SetGlyphType(2)  # Sphere glyph
    markups_node.GetDisplayNode().SetGlyphScale(2)
    markups_node.GetDisplayNode().SetColor(0,1,0)  # Green
    
    # Save to file
    output_path = os.path.join(output_dir, f"{name}.mrk.json")
    slicer.util.saveNode(markups_node, output_path)
    print(f"✅ Saved entry point markups to {output_path}")
