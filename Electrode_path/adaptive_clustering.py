...
def targeted_trajectory_refinement(trajectories, expected_contact_counts=[5, 8, 10, 12, 15, 18], 
    ...
    for i, traj1 in enumerate(merge_candidates):
        ...
        if best_match:
            ...
            print(f"Merged: {traj1['cluster_id']} + {traj2['cluster_id']} = {merged_traj['cluster_id']} (Score: {score:.2f})")
            
            # Add information about the merge distance
            if 'merge_distance' in merged_traj:
                print(f"  → Merge distance: {merged_traj['merge_distance']:.2f}mm")
            
            # Add information about entry points
            if 'entry_point' in merged_traj and merged_traj['entry_point'] is not None:
                print(f"  → Preserving entry point")
        else:
    ...
    for traj in split_candidates:
        ...
        if split_result['success']:
            ...
            # Add information about the split points if available
            if 'split_points' in split_result:
                print(f"  → Split at indices: {split_result['split_points']}")
            
            # Add information about entry points
            entry_point_count = sum(1 for t in split_result['trajectories'] if 'entry_point' in t and t['entry_point'] is not None)
            if entry_point_count > 0:
                print(f"  → {entry_point_count} sub-trajectories have entry points")
        else:
            # Couldn't split effectively, keep original
            final_trajectories.append(traj)
            print(f"Could not split {traj['cluster_id']}: {split_result['reason']}")
    ...
    # Calculate statistics for merged trajectories
    merged_count = len([t for t in final_trajectories if 'merged_from' in t])
    mean_merge_distance = np.mean([t.get('merge_distance', 0) for t in final_trajectories if 'merged_from' in t])
    
    # Calculate statistics for split trajectories
    ...
    return {
        ...
        'mean_merge_distance': float(mean_merge_distance) if not np.isnan(mean_merge_distance) else None,
        'split_count': split_count,
        'validation': validation_results
    }

def check_merge_compatibility(traj1, traj2, max_distance=15, max_angle_diff=20, min_distance_to_surface=5):
    """
    ...
    Args:
        ...
        min_distance_to_surface: Minimum distance to brain surface for endpoints
    ...
    # Determine which endpoints are closest, and check if they make sense for merging
    min_dist_idx = np.argmin(distances)
    
    # Check if these are entry points - entry points should be closest to surface
    # We'll use a heuristic: if these endpoints are marked as entry points, they shouldn't be merged
    if 'entry_point' in traj1 and traj1['entry_point'] is not None:
        if min_dist_idx in [0, 2] and np.array_equal(endpoints1[0], traj1['entry_point']):
            # First endpoint of traj1 is an entry point, but it's being connected - probably wrong
            return None
        if min_dist_idx in [1, 3] and np.array_equal(endpoints1[1], traj1['entry_point']):
            # Second endpoint of traj1 is an entry point, but it's being connected - probably wrong
            return None
    
    if 'entry_point' in traj2 and traj2['entry_point'] is not None:
        if min_dist_idx in [0, 1] and np.array_equal(endpoints2[0], traj2['entry_point']):
            # First endpoint of traj2 is an entry point, but it's being connected - probably wrong
            return None
        if min_dist_idx in [2, 3] and np.array_equal(endpoints2[1], traj2['entry_point']):
            # Second endpoint of traj2 is an entry point, but it's being connected - probably wrong
            return None
    ...
    # Check if the resulting merged trajectory maintains a consistent direction
    # (avoid merging trajectories that would create a sharp angle)
    if min_dist_idx == 0:  # endpoints1[0] and endpoints2[0] are closest
        merged_dir = endpoints2[1] - endpoints1[1]
    elif min_dist_idx == 1:  # endpoints1[0] and endpoints2[1] are closest
        merged_dir = endpoints2[0] - endpoints1[1]
    elif min_dist_idx == 2:  # endpoints1[1] and endpoints2[0] are closest
        merged_dir = endpoints2[1] - endpoints1[0]
    else:  # endpoints1[1] and endpoints2[1] are closest
        merged_dir = endpoints2[0] - endpoints1[0]
    
    merged_dir = merged_dir / np.linalg.norm(merged_dir)
    
    # Calculate angle between merged direction and original directions
    merged_angle1 = np.degrees(np.arccos(np.clip(np.abs(np.dot(merged_dir, dir1)), -1.0, 1.0)))
    merged_angle2 = np.degrees(np.arccos(np.clip(np.abs(np.dot(merged_dir, dir2)), -1.0, 1.0)))
    
    # If merged direction is very different from original, don't merge
    if merged_angle1 > max_angle_diff * 1.5 or merged_angle2 > max_angle_diff * 1.5:
        return None
    
    # Compute compatibility score (lower is better)
    # Add angle consistency to the score
    angle_consistency = (merged_angle1 + merged_angle2) / 2
    score = min_distance + angle * 0.5 + angle_consistency * 0.3
...
def merge_trajectories(traj1, traj2):
    ...
    # If we have entry points, make sure we preserve them
    entry_point1 = traj1.get('entry_point')
    entry_point2 = traj2.get('entry_point')
    
    # Determine the new entry point
    new_entry_point = None
    if entry_point1 is not None and entry_point2 is not None:
        # If both have entry points, choose the one that's more likely to be the true entry
        # (e.g., the one closer to the skull surface)
        if 'entry_to_surface_dist' in traj1 and 'entry_to_surface_dist' in traj2:
            if traj1['entry_to_surface_dist'] < traj2['entry_to_surface_dist']:
                new_entry_point = entry_point1
            else:
                new_entry_point = entry_point2
        else:
            # Simple heuristic: choose the entry point that's "first" in the trajectory
            # This is based on the assumption that entry points come before other contacts
            if idx1 == 0 and idx2 == 1:  # endpoints1[0] connects to endpoints2[1]
                new_entry_point = entry_point1
            elif idx1 == 1 and idx2 == 0:  # endpoints1[1] connects to endpoints2[0]
                new_entry_point = entry_point2
            else:
                # Use the entry point from the trajectory with more contacts
                if traj1['electrode_count'] > traj2['electrode_count']:
                    new_entry_point = entry_point1
                else:
                    new_entry_point = entry_point2
    elif entry_point1 is not None:
        new_entry_point = entry_point1
    elif entry_point2 is not None:
        new_entry_point = entry_point2
    ...
    # If we have an entry point, make sure it's at the beginning of the trajectory
    if new_entry_point is not None:
        # Calculate distances to entry point
        dist1 = np.linalg.norm(new_endpoints[0] - np.array(new_entry_point))
        dist2 = np.linalg.norm(new_endpoints[1] - np.array(new_entry_point))
        
        # If the second endpoint is closer to the entry point, swap endpoints
        if dist2 < dist1:
            new_endpoints = [new_endpoints[1], new_endpoints[0]]
    
    merged_traj['endpoints'] = [new_endpoints[0].tolist(), new_endpoints[1].tolist()]
    merged_traj['entry_point'] = new_entry_point
    ...
    if 'sorted_coords' in traj1 and 'sorted_coords' in traj2:
        ...
        # Determine the correct order to combine the coordinates
        if idx1 == 0 and idx2 == 0:
            sorted_coords = np.concatenate((coords1[::-1], coords2[1:]))
        elif idx1 == 0 and idx2 == 1:
            sorted_coords = np.concatenate((coords1[::-1], coords2[::-1][1:]))
        elif idx1 == 1 and idx2 == 0:
            sorted_coords = np.concatenate((coords1, coords2[1:]))
        else:  # idx1 == 1 and idx2 == 1
            sorted_coords = np.concatenate((coords1, coords2[::-1][1:]))
        
        # If we have an entry point, ensure coordinates are ordered correctly
        if new_entry_point is not None:
            # Calculate projections along the new direction
            center = np.mean(sorted_coords, axis=0)
            projected = np.dot(sorted_coords - center, new_direction)
            sorted_indices = np.argsort(projected)
            sorted_coords = sorted_coords[sorted_indices]
        
        merged_traj['sorted_coords'] = sorted_coords.tolist()
    
    # Store the merge distances
    merged_traj['merge_distance'] = closest_pair[2]
    
    return merged_traj

def split_trajectory(traj, expected_contact_counts=[5, 8, 10, 12, 15, 18], max_angle_difference=30):
    """
    ...
    Args:
        ...
        expected_contact_counts: List of expected electrode contact counts
        max_angle_difference: Maximum angle difference within a sub-trajectory (degrees)
    ...
    # Add directional analysis to look for significant direction changes
    # Calculate local directions between consecutive contacts
    if len(coords) > 2:
        local_dirs = []
        for i in range(1, len(coords)):
            dir_vec = coords[i] - coords[i-1]
            local_dirs.append(dir_vec / np.linalg.norm(dir_vec))
        
        # Calculate angles between consecutive directions
        angles = []
        for i in range(1, len(local_dirs)):
            dot_product = np.dot(local_dirs[i], local_dirs[i-1])
            angle = np.degrees(np.arccos(np.clip(dot_product, -1.0, 1.0)))
            angles.append(angle)
        
        # Identify potential split points based on angle changes
        potential_split_indices = []
        for i, angle in enumerate(angles):
            if angle > max_angle_difference:
                potential_split_indices.append(i + 1)  # +1 because angles are between points
        
        # If we found potential split points, use them to guide the clustering
        if potential_split_indices:
            print(f"Found {len(potential_split_indices)} potential split points based on direction changes")
            # Try to use these split points if they result in reasonable sub-trajectories
            if len(potential_split_indices) == len(best_combination) - 1:
                split_indices = [0] + potential_split_indices + [len(coords)]
                split_trajectories = []
                ...
                for i in range(len(split_indices) - 1):
                    ...
                    sub_coords = coords[start_idx:end_idx]
                    if len(sub_coords) >= 3:  # Need at least 3 points
                        ...
                        # Handle entry point - assign to the first sub-trajectory only
                        if i == 0 and 'entry_point' in traj and traj['entry_point'] is not None:
                            sub_traj['entry_point'] = traj['entry_point']
                        else:
                            sub_traj['entry_point'] = None
                ...
                # If we successfully created sub-trajectories using direction changes,
                # return these instead of using clustering
                if len(split_trajectories) >= 2:
                    return {
                        'success': True,
                        'reason': 'Split based on direction changes',
                        'trajectories': split_trajectories,
                        'n_trajectories': len(split_trajectories),
                        'split_points': potential_split_indices
                    }
    
    # If directional analysis didn't provide usable split points, continue with clustering
    ...
    for i, label in enumerate(unique_labels):
        ...
        # Handle entry points
        # If we have an entry point in the original trajectory, check if it belongs to this sub-trajectory
        if 'entry_point' in traj and traj['entry_point'] is not None:
            entry_point = np.array(traj['entry_point'])
            min_dist = float('inf')
            for point in sub_coords:
                dist = np.linalg.norm(point - entry_point)
                if dist < min_dist:
                    min_dist = dist
            
            # If the entry point is close to one of the points in this sub-trajectory,
            # assign it to this sub-trajectory
            if min_dist < 5.0:  # Threshold for considering the entry point part of this sub-trajectory
                sub_traj['entry_point'] = traj['entry_point']
            else:
                sub_traj['entry_point'] = None
        else:
...
def visualize_trajectory_refinement(coords_array, original_trajectories, refined_trajectories, refinement_results):
    """
    Create visualization showing the results of trajectory refinement with enhanced entry point display.
    ...
    for i, traj in enumerate(original_trajectories):
        ...
        # Plot entry point if available
        if 'entry_point' in traj and traj['entry_point'] is not None:
            entry_point = np.array(traj['entry_point'])
            ax1.scatter(entry_point[0], entry_point[1], entry_point[2],
                       color='red', marker='*', s=150, alpha=0.9)
            ax1.text(entry_point[0], entry_point[1], entry_point[2],
                   f"Entry {traj['cluster_id']}", color='red', fontsize=8)
    ...
    for i, traj in enumerate(refined_trajectories):
        ...
        if 'entry_point' in traj and traj['entry_point'] is not None:
            entry_point = np.array(traj['entry_point'])
            ax2.scatter(entry_point[0], entry_point[1], entry_point[2],
                       color='red', marker='*', s=150, alpha=0.9)
            
            # Draw line from entry point to first contact
            first_contact = endpoints[0]  # Assuming entry point is at beginning
            ax2.plot([entry_point[0], first_contact[0]],
                    [entry_point[1], first_contact[1]],
                    [entry_point[2], first_contact[2]],
                    '--', color='red', linewidth=1.5, alpha=0.8)
            
            ax2.text(entry_point[0], entry_point[1], entry_point[2],
                   f"Entry {traj['cluster_id']}", color='red', fontsize=8)
    ...
    entry_points_before = sum(1 for t in original_trajectories if 'entry_point' in t and t['entry_point'] is not None)
    entry_points_after = sum(1 for t in refined_trajectories if 'entry_point' in t and t['entry_point'] is not None)
    ...
    stats_text = (
        ...
        f"- Entry points before: {entry_points_before}\n"
        f"- Entry points after: {entry_points_after}"
    )
    
    # Add merge distance info if available
    if 'mean_merge_distance' in refinement_results and refinement_results['mean_merge_distance'] is not None:
        stats_text += f"\n- Mean merge distance: {refinement_results['mean_merge_distance']:.2f}mm"
    
    fig.text(0.01, 0.05, stats_text, fontsize=12, 
            bbox=dict(facecolor='white', alpha=0.8))
    
    # Create legend for special markers
    from matplotlib.lines import Line2D
    legend_elements = [
        Line2D([0], [0], color='k', marker='o', linestyle='', label='Original', markersize=8),
        Line2D([0], [0], color='k', marker='s', linestyle='', label='Merge candidate', markersize=8),
        Line2D([0], [0], color='k', marker='*', linestyle='', label='Split candidate/Split trajectory', markersize=8),
        Line2D([0], [0], color='k', marker='^', linestyle='', label='Merged trajectory', markersize=8),
        Line2D([0], [0], color='red', marker='*', linestyle='', label='Entry point', markersize=8)
    ]
...
def validate_trajectory_entry_points(trajectories, coords_array=None, brain_surface_points=None, max_distance=5.0):
    """
    Validate that entry points are properly assigned to trajectories and are at the correct end.
    
    Args:
        trajectories: List of trajectory dictionaries
        coords_array: Array of all electrode coordinates (for context)
        brain_surface_points: Array of brain surface points (if available)
        max_distance: Maximum allowed distance between entry point and closest trajectory point
    ...
    validation_results = {
        'trajectories_with_entry_points': 0,
        'valid_entry_positions': 0,
        'corrected_entry_positions': 0,
        'invalid_entry_points': 0,
        'details': {}
    }
    
    # Process each trajectory
    for traj_idx, traj in enumerate(trajectories):
        if 'entry_point' not in traj or traj['entry_point'] is None:
            # No entry point for this trajectory
            continue
            
        validation_results['trajectories_with_entry_points'] += 1
        entry_point = np.array(traj['entry_point'])
        endpoints = np.array(traj['endpoints'])
        
        # Calculate distances from entry point to endpoints
        dist_to_first = np.linalg.norm(entry_point - endpoints[0])
        dist_to_last = np.linalg.norm(entry_point - endpoints[1])
        
        # Get all coordinates for this trajectory if available
        traj_points = None
        if 'sorted_coords' in traj:
            traj_points = np.array(traj['sorted_coords'])
        elif coords_array is not None and 'cluster_id' in traj:
            # Filter coords_array to get only points for this trajectory
            # This would require additional information about the clustering
            pass
        
        # Find closest point in trajectory to entry point
        min_distance = min(dist_to_first, dist_to_last)
        closest_endpoint_idx = 0 if dist_to_first < dist_to_last else 1
        
        if traj_points is not None:
            # Check if any point in the trajectory is closer to the entry point
            distances = np.linalg.norm(traj_points - entry_point.reshape(1, -1), axis=1)
            closest_point_idx = np.argmin(distances)
            min_point_distance = distances[closest_point_idx]
            
            if min_point_distance < min_distance:
                min_distance = min_point_distance
        
        # Check if entry point is valid (close enough to trajectory)
        if min_distance > max_distance:
            # Entry point is too far from trajectory
            validation_results['invalid_entry_points'] += 1
            validation_results['details'][traj_idx] = {
                'trajectory_id': traj['cluster_id'],
                'status': 'invalid',
                'distance': float(min_distance),
                'reason': 'Entry point too far from trajectory'
            }
            continue
        
        # Check if entry point is at the correct end of trajectory
        # In SEEG, entry points should be at the beginning of the trajectory
        # (the point closest to the skull/surface)
        correct_position = True
        
        if brain_surface_points is not None:
            # If we have brain surface points, check which endpoint is closer to the surface
            surface_distances = []
            for endpoint in endpoints:
                distances_to_surface = np.linalg.norm(brain_surface_points - endpoint.reshape(1, -1), axis=1)
                min_surface_dist = np.min(distances_to_surface)
                surface_distances.append(min_surface_dist)
            
            # Endpoint closer to surface should be at index 0 if the trajectory is ordered correctly
            surface_closer_idx = np.argmin(surface_distances)
            
            if closest_endpoint_idx != surface_closer_idx:
                correct_position = False
        elif closest_endpoint_idx != 0:
            # If no surface points, assume entry point should be at the first endpoint
            correct_position = False
        
        if correct_position:
            validation_results['valid_entry_positions'] += 1
            validation_results['details'][traj_idx] = {
                'trajectory_id': traj['cluster_id'],
                'status': 'valid',
                'distance': float(min_distance)
            ...
            # Entry point is at the wrong end of trajectory - needs correction
            validation_results['corrected_entry_positions'] += 1
            
            # Swap endpoints to correct the trajectory direction
            traj['endpoints'] = [traj['endpoints'][1], traj['endpoints'][0]]
            
            # Reverse direction vector
            traj['direction'] = [-d for d in traj['direction']]
            
            # Update sorted coordinates if available
            if 'sorted_coords' in traj:
                traj['sorted_coords'] = traj['sorted_coords'][::-1]
            
            validation_results['details'][traj_idx] = {
                'trajectory_id': traj['cluster_id'],
                'status': 'corrected',
                'distance': float(min_distance),
                'reason': 'Entry point at wrong end of trajectory'
            }
    
    # Calculate percentages
    total_with_entry = validation_results['trajectories_with_entry_points']
    if total_with_entry > 0:
        validation_results['valid_percentage'] = (validation_results['valid_entry_positions'] / total_with_entry) * 100
        validation_results['corrected_percentage'] = (validation_results['corrected_entry_positions'] / total_with_entry) * 100
        validation_results['invalid_percentage'] = (validation_results['invalid_entry_points'] / total_with_entry) * 100
...
def main(use_combined_volume=True, use_original_reports=True, detect_duplicates=True, 
    ...
    try:
        ...
        electrodes_volume = slicer.util.getNode('P2_electrode_mask_success')  # Using your renamed node
        brain_volume = slicer.util.getNode("patient2_mask_5")
        
        # Create output directories
        base_dir = r"C:\Users\rocia\Downloads\TFG\Cohort\Centroids\P2_BoltHeadandpaths_contactsXpath_SPACING_dealing_with_problems_success_mask"
        ...
        # Create variable to store brain surface points if available (for trajectory validation)
        brain_surface_points = None
        if brain_volume:
            try:
                vertices, _ = get_surface_from_volume(brain_volume)
                if len(vertices) > 0:
                    brain_surface_points = convert_surface_vertices_to_ras(brain_volume, vertices)
                    print(f"Extracted {len(brain_surface_points)} brain surface points for validation.")
            except Exception as e:
                print(f"Could not extract brain surface: {e}")
        ...
        if refine_trajectories:
            ...
            # Apply enhanced targeted trajectory refinement
            ...
            # Add information about merge distances if available
            if 'mean_merge_distance' in refinement_results and refinement_results['mean_merge_distance'] is not None:
                print(f"- Mean merge distance: {refinement_results['mean_merge_distance']:.2f}mm")
            
            # After refinement, validate entry points if present
            if any('entry_point' in traj and traj['entry_point'] is not None for traj in refinement_results['trajectories']):
                print("Validating entry point positions...")
                entry_point_validation = validate_trajectory_entry_points(
                    refinement_results['trajectories'],
                    coords_array=coords_array,
                    brain_surface_points=brain_surface_points
                )
                
                refinement_results['entry_point_validation'] = entry_point_validation
                
                # Print validation results if any entry points were found
                if entry_point_validation['trajectories_with_entry_points'] > 0:
                    print(f"Entry point validation results:")
                    print(f"- Trajectories with entry points: {entry_point_validation['trajectories_with_entry_points']}")
                    print(f"- Valid positions: {entry_point_validation['valid_entry_positions']} "
                          f"({entry_point_validation.get('valid_percentage', 0):.1f}%)")
                    print(f"- Corrected positions: {entry_point_validation['corrected_entry_positions']} "
                          f"({entry_point_validation.get('corrected_percentage', 0):.1f}%)")
                    print(f"- Invalid entry points: {entry_point_validation['invalid_entry_points']} "
                          f"({entry_point_validation.get('invalid_percentage', 0):.1f}%)")
            ...
            # Update refinement results in all_results
            all_results['trajectory_refinement'] = refinement_results
            ...
            if use_original_reports:
                ...
                # Create enhanced visualization comparing original and refined trajectories
        ...
        # Add trajectory refinement summary
        if refine_trajectories and 'trajectory_refinement' in integrated_results:
            ...
            # Add information about merge distances if available
            if 'mean_merge_distance' in refinement and refinement['mean_merge_distance'] is not None:
                print(f"- Mean merge distance: {refinement['mean_merge_distance']:.2f}mm")
            
            # Add entry point validation summary if available
            if 'entry_point_validation' in refinement and refinement['entry_point_validation']['trajectories_with_entry_points'] > 0:
                entry_validation = refinement['entry_point_validation']
                print(f"- Entry point validation: {entry_validation['valid_entry_positions']} valid, "
                      f"{entry_validation['corrected_entry_positions']} corrected, "
                      f"{entry_validation['invalid_entry_points']} invalid")
...
if __name__ == "__main__":
    ...
    # # If you want to further analyze results, you can access specific components
    # if 'trajectory_refinement' in results.get('integrated_analysis', {}):
    #     refinement = results['integrated_analysis']['trajectory_refinement']
        
    #     # Get counts of merged and split trajectories
    #     merged_count = refinement['merged_count']
    #     split_count = refinement['split_count']
        
    #     # Get details on merge distances
    #     if 'mean_merge_distance' in refinement:
    #         mean_merge_distance = refinement['mean_merge_distance']
    #         print(f"Average merge distance: {mean_merge_distance:.2f}mm")
        
    #     # Print entry point validation statistics
    #     if 'entry_point_validation' in refinement:
    #         entry_validation = refinement['entry_point_validation']
    #         print(f"Entry point validation: {entry_validation['valid_percentage']:.1f}% valid, "
    #               f"{entry_validation['corrected_percentage']:.1f}% corrected")
            
    #     # Get final trajectories
    #     final_trajectories = results['integrated_analysis']['trajectories']
    #     print(f"Final trajectory count: {len(final_trajectories)}")
        
    #     # Check entry point distribution
    #     entry_count = sum(1 for t in final_trajectories if 'entry_point' in t and t['entry_point'] is not None)
    #     print(f"Trajectories with entry points: {entry_count} ({entry_count/len(final_trajectories)*100:.1f}%)")
